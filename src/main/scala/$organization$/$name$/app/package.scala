...
package object app {
  ...
  def buildConfig(config: Config): IO[VebConfig] =
    IO {
      ...
      VebConfig(threadPool, amqp, db, httpServer, httpClient)
    }

  def httpClient(config: VebConfig)(implicit ec: ExecutionContext, cs: ContextShift[IO]): Resource[IO, Client[IO]] =
  ...
  def mainThreadPool(config: VebConfig): Resource[SyncIO, ExecutorService] =
    Resource
      .make(
        for {
  ...
  def dbConnectionThreadPool(config: VebConfig): Resource[IO, ExecutorService] =
    Resource
      .make(
        for {
  ...
  def dbTransactionThreadPool(config: VebConfig): Resource[IO, ExecutorService] =
    Resource
      .make(
        for {
  ...
  def blazeServerBuilder(config: VebConfig)(implicit timer: Timer[IO], cs: ContextShift[IO], ec: ExecutionContext): BlazeServerBuilder[IO] =

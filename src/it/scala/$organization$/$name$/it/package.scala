...
import $organization$.$name$.app.{App, VebConfig}
...
package object it {
  ...
  def withApp[A](testData: TestData = TestData.empty, amqp: Option[AmqpClientTest[IO]] = None)(test: (TestApp, TestData) => IO[A]): A =
    (for {
      ...
      amqpClient     <- AmqpClientTest[IO](IO.ioConcurrentEffect(cs), timer, cs).clientStrict()
      ...
      App(ec, cs, timer, transactor, client, amqpClient, conf)
        ...
        val testApp = new TestApp {
          ...
          override def config: VebConfig        = app.config
          override def appClient: Client[IO]    = Client.fromHttpApp(app.routes)
        }
        test(
          testApp,
          testData
